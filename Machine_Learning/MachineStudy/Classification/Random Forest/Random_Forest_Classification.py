...
# Treinando o algoritimo da forest de classificacao aleatoria
...
plt.title('Random Forest Classificacao (Tranding training)')
plt.xlabel('Age')
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
    c= ListedColormap(('red', 'green'))(i), label= j)
plt.title('Random Forest Classificacao (Tranding set)')
plt.xlabel('Age')
